...
#ifdef frame
const uint8_t frame_mini[] PROGMEM = {
...
// Private functions
uint8_t Oled_reverse(uint8_t b) {                      // reverse bitorder
   b = (b & 0xF0) >> 4 | (b & 0x0F) << 4;
   b = (b & 0xCC) >> 2 | (b & 0x33) << 2;
   b = (b & 0xAA) >> 1 | (b & 0x55) << 1;
   return b;
...
void Oled_clear() {
  ...
  for (p = 0; p < 8; p++) {
    ...
    if (p < 4) {
        pxbyte = pgm_read_byte(frame_mini[pxbyte]);    
      } else {
        pxbyte = Oled_reverse(pgm_read_byte(frame_mini[pxbyte]));  
      }
